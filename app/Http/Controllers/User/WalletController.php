...
use App\Services\TP_Integrations\FlutterwaveService;
use App\Services\TP_Integrations\RedbillerService;
...
class WalletController extends Controller
{
    ...
    public function __construct(WalletService $walletService, FlutterwaveService $flutterwave, RedbillerService $redbiller)
    {
    ...
    //create vba with the flutterwave api
    public function createVba(Request $request)
    {
        $result = $this->flutterwave->createVbaService($request->user());
        if ($result['success']) {
            VirtualBankAccount::create([
                ...
                'currency' => $result['data']->currency ?? $request->user()->wallet->currency,
                'status' => true,
                'meta' => $result['data'],
